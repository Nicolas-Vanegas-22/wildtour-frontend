...
import {
  ...
  USED_FOR_OPTIONS,
  SERVICE_RECOMMENDED_FOR_OPTIONS
...
interface ServiceReviewModalProps {
  ...
  onSubmit: (review: CreateServiceReviewRequest) => Promise<void>;
  loading?: boolean;
}
...
export const ServiceReviewModal: React.FC<ServiceReviewModalProps> = ({
  ...
  const [formData, setFormData] = useState<Partial<CreateServiceReviewRequest>>({
    serviceId,
    serviceName,
    providerName,
    serviceType,
    ...
    groupSize: undefined,
    recommendedFor: [],
    rating: {} as ServiceRatingCategories
  });
  ...
  const validateStep = (step: string): boolean => {
    ...
    switch (step) {
      case 'basic':
        if (!formData.serviceDate) newErrors.serviceDate = 'La fecha del servicio es requerida';
        if (!formData.usedFor) newErrors.usedFor = 'Indica para qué usaste el servicio';
        if (serviceType === 'transportation' && (!formData.duration || formData.duration < 1)) {
          newErrors.duration = 'La duración debe ser mayor a 0';
        }
        break;

      case 'rating':
        if (!formData.rating || Object.keys(formData.rating).length === 0) {
          newErrors.rating = 'Por favor, califica el servicio';
        } else {
          const ratingValues = Object.values(formData.rating);
          if (ratingValues.some(val => val === 0)) {
            newErrors.rating = 'Califica todos los aspectos del servicio';
          }
        }
        break;
  ...
  const renderStepIndicator = () => {
    const steps = [
      ...
      { key: 'review', label: 'Comentarios', icon: getServiceTypeIcon() }
    ];
  ...
  const renderBasicStep = () => (
    <div className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className={cn(
              ...
              errors.serviceDate && 'border-error-500'
            )}
            max={new Date().toISOString().split('T')[0]}
          />
          {errors.serviceDate && <p className="text-error-500 text-sm mt-1">{errors.serviceDate}</p>}
        </div>
        ...
        {serviceType === 'transportation' && (
          <div>
            ...
            <input
              ...
              className={cn(
                'w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500',
                errors.duration && 'border-error-500'
              )}
            />
            {errors.duration && <p className="text-error-500 text-sm mt-1">{errors.duration}</p>}
          </div>
        )}

        {(serviceType === 'transportation' || serviceType === 'equipment') && (
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-2">
              Tamaño del Grupo
            </label>
      ...
      <div>
        <label className="block text-sm font-medium text-neutral-700 mb-2">
          ¿Para qué utilizaste el servicio? *
        </label>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {USED_FOR_OPTIONS.map((option) => (
            <button
              key={option.value}
              type="button"
              onClick={() => setFormData(prev => ({ ...prev, usedFor: option.value }))}
              className={cn(
                'flex flex-col items-center p-3 border-2 rounded-lg transition-colors',
                formData.usedFor === option.value
        ...
        {errors.usedFor && <p className="text-error-500 text-sm mt-1">{errors.usedFor}</p>}
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-neutral-900 mb-2">
          Califica tu experiencia con {serviceName}
        </h3>
        <p className="text-neutral-600">
          Ayuda a otros usuarios conociendo tu experiencia con este {getServiceTypeLabel().toLowerCase()}
        </p>
      </div>
  ...
  const renderMediaStep = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-neutral-600">
          Comparte imágenes de tu experiencia para ayudar a otros usuarios
        </p>
      </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <input
          ...
          placeholder={`Ej: Excelente ${getServiceTypeLabel().toLowerCase()} en ${providerName}`}
          className={cn(
            'w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500',
      ...
      <div>
        ...
        <textarea
          ...
          rows={6}
          placeholder={`Describe tu experiencia con este ${getServiceTypeLabel().toLowerCase()}, qué te gustó más, qué mejorarías...`}
          className={cn(
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {SERVICE_RECOMMENDED_FOR_OPTIONS.map((option) => {
